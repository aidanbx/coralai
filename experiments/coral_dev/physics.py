...
def activate_outputs(substrate):
    ...
    # Tanh signed trade: positive raw diff → invest, negative → liquidate.
    # Unlike softmax, cells can express "do nothing" (both near zero when raw≈0)
    # and the magnitude of each is independent, not forced to sum to 1.
    raw_trade = mem[0, inds.acts_invest] - mem[0, inds.acts_liquidate]
    trade = torch.tanh(raw_trade)
    mem[0, inds.acts_invest]    = torch.relu(trade)   # invest rate ∈ [0, 1]
    mem[0, inds.acts_liquidate] = torch.relu(-trade)  # liquidate rate ∈ [0, 1]
