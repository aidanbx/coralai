"""
Coral experiment runner.

Runs the spatial NEAT coral simulation with optional GUI, profiling, or
benchmark modes.  All three previous entry points (coral_runner_space.py,
profile_coral_space_gui.py, benchmark_coral_space.py) are unified here.
...
class CoralVis(Visualization):
    ...
    def render_opt_window(self):
        ...
        with self.gui.sub_window("Options", 0.05, 0.05, opt_w, opt_h) as sub_w:
            ...
            px = int(pos[0] * self.w) % self.w
            py = int(pos[1] * self.h) % self.h
            sub_w.text(
                f"GENOME: {self.substrate.mem[0, inds.genome, px, py]:.2f}\n"
                f"Energy: {self.substrate.mem[0, inds.energy, px, py]:.2f}\n"
                f"Infra:  {self.substrate.mem[0, inds.infra, px, py]:.2f}")
            ...
            sub_w.text(f"Total E+I: {tot_e + tot_i:.1f}")
            sub_w.text(f"Energy%: {100 * tot_e / (tot_e + tot_i):.1f}%")
            sub_w.text(f"Energy offset: {self.evolver.energy_offset:.3f}")
