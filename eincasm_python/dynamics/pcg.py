...
def perlin2d(
    ...
    for x in range(width):
        for y in range(height):
            value = noise.pnoise2(
                (x + x_offset) / frequency,
                (y + y_offset) / frequency,
                octaves=octaves,
...
def discretize_levy_dust(shape: tuple, dust: np.array) -> np.array:
    """Discretize a levy dust cloud into a grid of shape shape, such that each position
    in the grid is the number of points in the cloud that fall in that position
...
def levy_dust(shape: tuple, points: int, alpha: float, beta: float) -> np.array:
    angle = uniform.rvs(size=(points,), loc=0.0, scale=2.0 * np.pi)
...
def random_signal(
    ...
    start_periods = (max_start_period - min_start_period) * torch.rand(
        num_components
    ) + min_start_period
    ...
    return (
        lambda t: sum(
            [
                amps[i] * torch.sin(freqs[i] * t + start_periods[i])
...
class Resource:
    ...
    def __init__(self, resource_id, regen_func, metadata=None, dispersal_func=None):
        self.resource_id = resource_id
        self.regen_func = regen_func
        default_metadata = {"resource_id": resource_id}
        if metadata is None:
            metadata = {}
